...
Your team should setup the staging environment on one team member's namespace. Choose one members' CloudApps namespace to setup the production staging environment. The other team member(s) will be added to this namespace as collaborator(s) so everyone has access.
...
Under the Red Hat Logo, you should see "Developer" in a drop down and to the right "Project: " followed by your ONYEN (or your team member's ONYEN whose account handles deployment).

## Using the `oc` Command-line Tool to Administer OpenShift

The `oc` tool is included in the `csxl` developer container. As long as you are successfully running Docker locally and connected to a campus network or VPN, you will be able to use `oc` from your container. However, if you are making use of CodeSpaces, you will need to follow [these instructions](/exercises/oc-host-install.html) for installing `oc` onto your host machine so that you can access the OpenShift cluster from VPN/campus rather than the cloud container.
...
Go ahead and log-in. Remember: off-campus access requires VPN!
...
1. Add team members to your course workspace
  ...
  * For each team member, with their `onyen`:
    ...
    3. Be sure the project you are in in CloudApps is `comp590-140-24fa-ONYEN`
...
```bash
oc new-app python:3.11~git@github.com:comp423-24f/<your-final_repo_name>.git#stage \
  --source-secret=comp590-final-project-deploykey \
  --name=final-project \
  --strategy=docker \
  --env=MODE=development \
  --env=HOST=csxl-team-<TEAM NUMBER>-comp590-24f.apps.unc.edu
```
...
oc create route edge \
  ...
  --hostname=csxl-team-<TEAM NUMBER>-comp590-24f.apps.unc.edu
```
